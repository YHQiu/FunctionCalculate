...
/**
 For lowest common multiple calculate
 */
class LowestCommonMultiple: FunctionCalculateProtocol {
    ...
    func calculateStringResultWithMultiParam(params: [Float]) -> String? {
        ...
        var firstDivisorNums = divisorNumsArray[0]
        var results:[Int] = []
        
        //get results by elimination.
        ...
        //get left results
        for divisorNums in divisorNumsArray{
            for element in divisorNums{
                if element.primeNum > 1{
                    var isHave:Bool = false
                    for currentNum in results{
                        if currentNum == element.primeNum{
                            isHave = true
                        }
                    }
                    if !isHave {
                       results.append(element.primeNum)
                    }
                }
